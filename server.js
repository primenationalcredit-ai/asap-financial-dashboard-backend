...
let tokens = { access_token: null, refresh_token: null, expires_at: null, realm_id: null };
...
async function qbRequest(endpoint) {
    ...
    const response = await fetch(API_BASE + '/' + tokens.realm_id + endpoint, {
...
async function fetchFinancialData() {
    ...
    var result = {
        ...
        transactions: (purchases.QueryResponse.Purchase || []).slice(0, 10).map(function(p, i) { return { id: i, date: p.TxnDate, description: p.PrivateNote || 'Expense', category: 'Expense', amount: -p.TotalAmt, type: 'expense' }; }),
        timestamp: Date.now()
    };
